    ...
    switch(type){
      case 'text':{
        ...
        if(elements.length > 1 && elements[1] != ''){
          ...
          let date;
          let result;
          console.log("-->:",ruleDate[1].exec(elements[0]))
          if((result = ruleDate[0].exec(elements[0])?ruleDate[0].exec(elements[0]).slice(1,7).map(item => {if(!(~~item)) return parseInt(item)}):false)){
            if(result[3] === '下午'){
              //12小时计时方法下午时间
          ...
          }else if((result = ruleDate[1].exec(elements[0])?ruleDate[1].exec(elements[0]).slice(1,5).map(item => {if(!(~~item)) return parseInt(item)}):false)){
            if(result[0] === '今天'){
              date = new Date()
          ...
          respData = buildXmlMsg(req.body,date.toLocalString())
        }else{
          respData = buildXmlMsg(req.body,'没有捕获到时间')
        }
        break;
      }
      default :{
